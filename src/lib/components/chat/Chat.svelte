<script lang="ts">
  ...
  import { chatCompleted, generateQueries, stopTask, getTaskIdsByChatId } from '$lib/apis';
  ...
  const loadChat = async () => {
    ...
    if (chat) {
      ...
      if (chatContent) {
        ...
        selectedModels =
          (chatContent?.models ?? undefined) !== undefined ? [chatContent.models[0] ?? ''] : [''];
  ...
  const submitPrompt = async (userPrompt, { _raw = false } = {}) => {
    ...
    chatFiles.push(
      ..._files.filter(
        (item) =>
          ['doc', 'text', 'note', 'folder', 'collection'].includes(item.type) ||
    ...
    await Promise.all(
      selectedModelIds.map(async (modelId, _modelIdx) => {
        ...
        if (model) {
          ...
          let responseMessageId = responseMessageIds[`${modelId}-${_modelIdx}`];
  ...
  const sendMessageSocket = async (model, _messages, _history, responseMessageId, _chatId) => {
    ...
    files.push(
      ...(userMessage?.files ?? []).filter(
        (item) =>
          ['doc', 'text', 'note', 'collection'].includes(item.type) ||
          (item.type === 'file' && !(item?.content_type ?? '').startsWith('image/'))
      )
    );
    // Remove duplicates
