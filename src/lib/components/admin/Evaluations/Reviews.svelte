          ...
          >
            ...
            <td class="px-3 py-1.5 text-right font-medium text-green-500 w-10">
              {#if model.total === 0}-{:else}
                <Tooltip content="{((model.positive / model.total) * 100).toFixed(1)}%">
            ...
            <td class="px-3 py-1.5 text-right font-medium text-red-500 w-10">
              {#if model.total === 0}-{:else}
                <Tooltip content="{((model.negative / model.total) * 100).toFixed(1)}%">
...
<div class="text-gray-500 text-xs mt-1.5 w-full flex justify-end">
  <div class="text-right">
    ...
    {$i18n.t(
      'Models with fewer reviews are pulled toward an average score until more data is collected.'
    )}
  </div>
</div>
